...
class MLB_Scrape:
    ...
    def get_games_by_date(self, date: str, sport_id: int = 11, game_type: list = ('R','P')) -> list[int]:
        """
        Return a list of gamePk for a given sport (AAA=11) on a specific ET date.
        Includes postseason by default.
        """
        ...
        params = {
            "sportId": sport_id,
            "date": date,               # YYYY-MM-DD (ET 'officialDate')
            "gameTypes": game_type_str, # e.g. "R,P"
            "language": "en"
        }
    ...
    def get_player_games_list_safe(
        self,
        player_id: int,
        season: int,
        date: str,
        ...
        """
        Try the people→stats(gameLog) hydrate. If that returns no games (common for MLB rehabs),
        fall back to schedule-by-date for AAA and filter downstream by pitcher_id.
        """
        try:
        ...
        # Fallback for rehab/AAA cases
        return self.get_games_by_date(date=date, sport_id=sport_id, game_type=game_type)
