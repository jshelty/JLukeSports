        ...
        for data in data_list:
            try:
                for ab_id in range(len(data['liveData']['plays']['allPlays'])):
                    ...
                    home_team_lineup = data['liveData']['linescore']['teams']['home']['lineup']
                    away_team_lineup = data['liveData']['linescore']['teams']['away']['lineup']
                    
                    # Identify the catcher for the home and away teams
                    home_catcher = next((player for player in home_team_lineup if player['position']['abbreviation'] == 'C'), None)
                    away_catcher = next((player for player in away_team_lineup if player['position']['abbreviation'] == 'C'), None)

                    # Loop through the play events
                    for n in range(len(ab_list['playEvents'])):
                        ...
                        # Add catcher information based on team
                        if ab_list['about']['isTopInning']:  # Top inning means the away team is batting
                            catcher_name.append(away_catcher['player']['fullName'] if away_catcher else None)
                        else:  # Bottom inning means the home team is batting
                            catcher_name.append(home_catcher['player']['fullName'] if home_catcher else None)
